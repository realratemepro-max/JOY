...
export const generateProfessorResetLink = functions
  .region('europe-west1')
  .https.onCall(async (data, context) => {
    ...
    if (sendEmail) {
      try {
        ...
        const subject = `Acesso ao Portal do Professor — ${siteName}`;
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:40px 40px 32px;">
          <h1 style="margin:0 0 8px;font-size:1.375rem;font-weight:600;color:#1a1a1a;">Olá, ${professorName || ''}!</h1>
          ...
          <div style="text-align:center;margin:32px 0;">
            <a href="${resetLink}" style="display:inline-block;background:#7c9a72;color:#ffffff;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:600;font-size:1rem;letter-spacing:0.01em;">
              Aceder ao Portal
            </a>
          </div>
        ...
        if (config.emailProvider === 'resend' && config.resendApiKey) {
          ...
          console.log('✅ Email sent via Resend, id:', resp.data?.id);
          emailSent = true;
        ...
        console.error('❌ Email send failed:', msg, err?.response?.data);
        emailError = msg;
      }
    }
